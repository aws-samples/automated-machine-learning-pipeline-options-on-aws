...
```bash
ml-sam$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ml-sam$ sam local start-api
ml-sam$ curl http://localhost:3000/classify_digit
```
...
```bash
ml-sam$ sam logs -n InferenceFunction --stack-name ml-sam --tail
```
...
```bash
aws cloudformation delete-stack --stack-name ml-sam
```
